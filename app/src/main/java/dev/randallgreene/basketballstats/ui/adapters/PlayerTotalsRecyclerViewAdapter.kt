...
class PlayerTotalsRecyclerViewAdapter internal constructor(context: Context)
    : RecyclerView.Adapter<PlayerTotalsRecyclerViewAdapter.SeasonShootingTotalsItemHolder>() {
    ...
    inner class SeasonShootingTotalsItemHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SeasonShootingTotalsItemHolder {
        val itemView = inflater.inflate(R.layout.stats_totals_layout, parent, false)
        return SeasonShootingTotalsItemHolder(itemView)
    }

    override fun getItemCount(): Int = items.size + 1

    @SuppressLint("SetTextI18n")
    override fun onBindViewHolder(holder: SeasonShootingTotalsItemHolder, position: Int) {
        ...
        if (position == 0) {
            ...
            holder.fieldGoalsAttemptedTextView.displayValue(currentItem.totalFieldGoalsAttempted)
            holder.threesTotalTextView.displayValue(currentItem.totalThreesMade)
            holder.threesAttemptedTextView.displayValue(currentItem.totalThreesAttempted)
            holder.freeThrowsTotalTextView.displayValue(currentItem.totalFreeThrowsMade)
            holder.freeThrowsAttemptedTextView.displayValue(currentItem.totalFreeThrowsAttempted)
